...
class UserBasedCF():
    ...
    def generate_dataset(self, filename, pivot=0.7):
        ''' load rating Datas and split it to training set and test set '''
        trainset_len = 0
        testset_len = 0

        for line in self.loadfile(filename):
            user, movie, rating, timestamp = line.split('\t')
            # split the Datas by pivot
    ...
    def evaluate(self):
        ...
        for i, user in enumerate(self.trainset):  # enumerate 函数用于遍历序列中的元素以及它们的下标：
            if i % 500 == 0:
...
if __name__ == '__main__':
    ratingfile = 'Datas/ml-100k/u.Datas'
    usercf = UserBasedCF()
